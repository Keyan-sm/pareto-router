...
from pareto_router import load_routerbench, run_benchmark  # noqa: E402
...
def main() -> None:
    ...
    for name, info in report.baselines.items():
        if info["kind"] == "single_model":
            ...
            ax.annotate(name.split("/")[-1], (info["cost"], info["quality"]),
    ...
    ax.set_title(f"pareto-router on RouterBench — {report.n_test:,} held-out queries, {len(report.models)} models")
